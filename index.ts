...
async function initialize() {
    ...
    if (keystorePath) {
        ...
        if (!password) {
            console.log(
                "[Security] KEYSTORE_PASSWORD not found in .env. Switching to manual input mode.",
            );
        ...
        try {
            ...
            console.error(
                "[Security] Failed to decrypt keystore. Please check your password and keystore file.",
            );
        ...
        console.warn(
            "[Security] WARNING: Initializing wallet from a plaintext PRIVATE_KEY. For production and larger funds, using an encrypted keystore is strongly recommended.",
        );
        baseWallet = new ethers.Wallet(privateKey, provider);
    } else {
        console.log(
            "[Security] No credentials found in .env. Switching to manual private key input mode.",
        );
    ...
    provider.on("block", async (blockNumber) => {
        ...
        try {
            ...
            if ((e as Error).message.includes("PROFIT_SECURED")) {
                ...
                console.log(
                    "[System] Entering STANDBY MODE. Will periodically check for reentry conditions.",
                );

                // 发送邮件通知，但不退出了
                await sendEmailAlert(
                    "Bot Standby: Profit Secured",
                    `Sold all ETH for USDC. Bot is now waiting for opportunities.`,
                );
                ...
                try {
                    ...
                    const priceStr =
                        configuredPool.token0.address.toLowerCase() ===
                    ...
                    console.error(
                        "[Standby] CRITICAL: Failed to record profit-taking price:",
                        priceError,
                    );
                    ...
                    await sendEmailAlert(
                        "Bot Standby Error",
                        "Profit secured, but failed to record current price. Manual restart required after dip.",
                    );
                }
...
async function onNewBlock(blockNumber: number) {
    let { tokenId, lastKnownUSDCBalance, stopLossTriggered, stopLossType, circuitBreakerPrice } =
        (await loadState()) as any;
    ...
    if (CIRCUIT_BREAKER_ENABLED) {
        ...
        if (!stopLossTriggered || stopLossType === "SOFT") {
            ...
            if (blockNumber % 20 === 0)
                console.log(
                    `[Monitor] Total Assets: $${totalValue.toFixed(2)} (Soft: $${CIRCUIT_BREAKER_SOFT_THRESHOLD} | Hard: $${CIRCUIT_BREAKER_HARD_THRESHOLD})`,
                );
            ...
            if (hardStopConfirmationCount > 0 || softStopConfirmationCount > 0) {
                console.warn(
                    `[CircuitBreaker] Danger Zone. HardCount: ${hardStopConfirmationCount}/3, SoftCount: ${softStopConfirmationCount}/3`,
                );
            }
    ...
    if (stopLossTriggered) {
        if (blockNumber % 50 === 0)
            console.log(
                `[StopLoss] Bot is in ${stopLossType} STOP_LOSS mode. Monitoring for revival...`,
            );

        // Revival Monitor: Check if price drops 10% below exit price (Good entry) or recovers
        if (circuitBreakerPrice) {
            const currentPrice = await getPrice();
            const targetEntry = circuitBreakerPrice * 0.9; // 10% drop
            ...
            let rsi4h = 50;
            try {
                [rsi15m, rsi4h] = await Promise.all([getEthRsi("15m"), getEthRsi("4h")]);
            } catch (e) {
            ...
            if (
                ...
                rsi15m < 30 &&
                Date.now() - lastAlertTime > 3600 * 1000
            ) {
                console.log(
                    `[StopLoss] Opportunity detected! Price ${currentPrice} < Target ${targetEntry} | RSI 15m: ${rsi15m} | RSI 4h: ${rsi4h}`,
                );
                await sendEmailAlert(
                    "Bot Revival Opportunity",
                    `Price dropped 10% below exit & RSI Oversold.\nCurrent: ${currentPrice}\nTarget: ${targetEntry}\nRSI (15m): ${rsi15m} (Panic)\nRSI (4h): ${rsi4h} (Trend)\n\nUpdate 'stopLossTriggered' to false in state to restart.`,
                );
    ...
    if (isProfitStandby) {
        if (profitSecuredPrice > 0) {
            ...
            const priceStr =
                configuredPool.token0.address.toLowerCase() === WETH_TOKEN.address.toLowerCase()
                    ? configuredPool.token0Price.toSignificant(6)
                    : configuredPool.token1Price.toSignificant(6);
            ...
            console.log(
                `[Standby] Checking for re-entry. Current Price: ${currentPrice}, Re-entry Target: < ${reentryPrice.toFixed(2)}`,
            );
            ...
            if (currentPrice < reentryPrice) {
                ...
                await sendEmailAlert(
                    "Bot Resuming",
                    `Price dropped to ${currentPrice.toFixed(2)}. Re-entering position.`,
                );
            ...
            console.warn(
                "[Standby] In standby mode but profitSecuredPrice is not set. Manual action may be needed.",
            );
        }
    }
    ...
    if (AUTO_INVEST_NEW_FUNDS) {
        ...
        if (lastKnownUSDCBalance === undefined || lastKnownUSDCBalance === "0") {
            ...
            if (tokenId === "0" && currentUSDCBalance >= AUTO_INVEST_THRESHOLD_USDC) {
                console.log(
                    `[Auto-Invest] Initial funds detected (${ethers.formatUnits(currentUSDCBalance, 6)} USDC). Triggering initial investment.`,
                );
                forceRebalance = true;
            } else {
            ...
            if (depositAmount >= AUTO_INVEST_THRESHOLD_USDC) {
                console.log(
                    `[Auto-Invest] New deposit of ${ethers.formatUnits(depositAmount, 6)} USDC detected. Scheduling a rebalance.`,
                );
                forceRebalance = true;
            }
        }
    ...
    if (!tokenId || tokenId === "0") {
        if (forceRebalance || !AUTO_INVEST_NEW_FUNDS) {
            ...
            console.log(
                `[Block ${blockNumber}] No active position and no new funds to invest. Waiting...`,
            );
        }
        return;
    }
...
async function calculateTotalAssets(tokenId: string): Promise<number> {
    ...
    let totalValue =
        Number(ethers.formatUnits(balUSDC, 6)) + Number(ethers.formatEther(balWETH)) * price;
            ...
            if (pos.liquidity > 0n) {
                ...
                try {
                    ...
                    if (
                        ...
                        feeVal =
                            parseFloat(ethers.formatUnits(fee1, 6)) +
                            parseFloat(ethers.formatUnits(fee0, 18)) * price;
                    } else {
                        // token0 is USDC, token1 is WETH
                        feeVal =
                            parseFloat(ethers.formatUnits(fee0, 6)) +
                            parseFloat(ethers.formatUnits(fee1, 18)) * price;
                    }
                    totalValue += feeVal;
                } catch (e) {
...
async function triggerCircuitBreaker(tokenId: string, currentVal: number, type: "SOFT" | "HARD") {
    const threshold =
        type === "HARD" ? CIRCUIT_BREAKER_HARD_THRESHOLD : CIRCUIT_BREAKER_SOFT_THRESHOLD;
    ...
    // [Strategy] High Priority Gas for Panic Exit
    // const panicOverrides = {
    //     maxPriorityFeePerGas: ethers.parseUnits("50", "gwei"), // Extreme Priority
    //     maxFeePerGas: ethers.parseUnits("1000", "gwei"),       // High Cap
    // };
    // console.log(`[CircuitBreaker] Using High Priority Gas: 50 Gwei Priority`);
    ...
    if (type === "HARD") {
        try {
            const balWETH = await getBalance(WETH_TOKEN, wallet);
            const keepAmount =
                (balWETH * BigInt(Math.floor(STOP_LOSS_KEEP_WETH_PERCENT * 100))) / 100n;
            const sellAmount = balWETH - keepAmount;

            if (sellAmount > 100000000000000n) {
                // > 0.0001 ETH
                console.log(
                    `[CircuitBreaker] HARD STOP: Selling ${ethers.formatEther(sellAmount)} WETH...`,
                );
    ...
    saveState({
        ...
        stopLossType: type,
        tokenId: "0",
        circuitBreakerPrice: exitPrice,
    });

    const actionMsg =
        type === "HARD"
            ? "Liquidity Removed, WETH Sold (Cash Out)."
            : "Liquidity Removed, Holding WETH (Wait for bounce).";
...
initialize().catch(async (e) => {
    ...
    await sendEmailAlert(
        "Bot Crashed",
        `The bot process exited unexpectedly.\nError: ${e.message}`,
    );
    process.exit(1);
});
